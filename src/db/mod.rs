...
pub async fn daily_reset(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    // Retrieve the date of the last reset from the database
    let last_reset: Option<String> = sqlx::query_scalar("SELECT value FROM game_state WHERE key = 'last_reset'")
        .fetch_one(pool).await.ok();

    // If the last reset was before today, perform the reset
    if last_reset.unwrap_or_default() < today {
        ...
        log_event(pool, "A new day dawns in the realm. All heroes feel refreshed.").await?;
...
///
/// This function:
/// 1. Normalizes the player name by trimming whitespace
/// 2. Securely hashes the password using Argon2 (if provided)
/// 3. Creates a new player record with default starting values
/// 4. Immediately return the created player (ensures it exists in DB)
///
/// # Parameters
///
/// * `pool` - The database connection pool
...
/// # Returns
///
/// `Ok(())` if the player was created successfully, or a `sqlx::Error` if:
...
/// Create a new player and immediately return the created player (ensures it exists in DB)
pub async fn create_player(pool: &PgPool, name: &str, password: &str) -> Result<Player, sqlx::Error> {
    ...
    let player = Player {
        ...
        last_login: row.try_get::<NaiveDateTime, _>("last_login")?, // ✅ Corrected
    };
...
/// # Returns
///
/// `Some(Player)` if a player with the given name exists, or `None` if no match is found.
///
/// # Security Note
///
...
pub async fn get_player_by_name(pool: &PgPool, name: &str) -> Option<Player> {
...
/// # Returns
///
/// `Some(Player)` if a player with the given ID exists, or `None` if no match is found.
pub async fn get_player_by_id(pool: &PgPool, player_id: i32) -> Option<Player> {
    sqlx::query_as::<_, Player>(
...
/// # Note
///
/// This function does not update the password field. To change a password,
/// use a dedicated function that properly hashes the new password.
...
pub async fn get_latest_events(pool: &PgPool, limit: i64) -> Result<Vec<(NaiveDateTime, String)>, sqlx::Error> {
    // Define a struct to represent the query result
    #[derive(sqlx::FromRow)]
    struct NewsEvent {
        date: NaiveDateTime,
        message: String,
    ...
    // and convert to a vector of tuples
...
/// # Returns
///
/// A vector of PlayerInfo structs for all alive players except the excluded one,
/// or a `sqlx::Error` if the database operation fails.
...
/// # Returns
///
/// A vector of PlayerInfo structs for the top players,
/// or a `sqlx::Error` if the database operation fails.
...
/// # Security
///
/// This function uses the Argon2 algorithm for password verification,
/// which is resistant to brute force attacks and timing attacks.
